...
public class updateSkills extends JFrame {
	...
	public updateSkills() {
		...
		JLabel lblNewLabel = new JLabel("These are the existing Skills, choose one to update:");
		lblNewLabel.setFont(new Font("Tahoma", Font.BOLD, 14));
		lblNewLabel.setBounds(31, 11, 480, 33);
		contentPane.add(lblNewLabel);
		
		JLabel lblNewLabel_1 = new JLabel("Enter the number of Skill you've chosen:");
		...
		for(int i=1; i<=CVEditor.getCV().getSections().get(1).getBulletList().size(); i++){
	    	String s=CVEditor.getCV().getSections().get(1).getBulletList().get(i-1).getContent1();
	        text += i+")"+" " +s+ "\r\n";
	    } 
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					if(CVEditor.getIndexItem() > CVEditor.getCV().getSections().get(1).getBulletList().size() || CVEditor.getIndexItem()-1<0){
						JOptionPane.showMessageDialog(null, "Please Enter a valid number");
						return;
					}
					else{
						BulletListItem s = CVEditor.getCV().getSections().get(1).getBulletList().get(CVEditor.getIndexItem()-1);
						CVEditor.setItem(s);
						dispose();
